...
int modify(int h1, int h2, int position){

    h1+=(h1 * h2 - position & h2);
    if(h1>10000||h1<-10000) h1/=76;//noriu isvengti overflow

    return h1;
};//*/

std::string hasher(std::string base, const int Hash_size){
    ...
    while(n-->0){ //while(n>0){ n--; ...
       ...
       //po compress iz>= 0 bet maziau nei Hash_size
        h[i]+=(modify( h[i], base[j], n)+
               modify( base[j], h[iz], i-j));
